...
parser = argparse.ArgumentParser(description="Evaluate the DiffC compression algorithm on an image or folder of images.")
parser.add_argument('--config', help="Path to the compression config file. For example, config/SD-1.5-no-prompt.yaml")
parser.add_argument('--image_path', default=None, help="Path to a single image to compress")
parser.add_argument('--image_dir', default=None, help="Path to a directory containing one or more images to compress")
parser.add_argument('--output_dir', help="Directory to output the compression results to.")
...
gaussian_channel_simulator = GaussianChannelSimulator(config.max_chunk_size, config.chunk_padding)
...
for image_path in image_paths:
    ...
    noise_prediction_model.configure(prompt, config.encoding_guidance_scale, img_width, img_height)
    ...
    noise_prediction_model.configure(prompt, config.denoising_guidance_scale, img_width, img_height)

    for noisy_recon, step_idx in zip(noisy_recons, noisy_recon_step_indices):
        
        bytes = gaussian_channel_simulator.compress_chunk_seeds(chunk_seeds_per_step[:step_idx+1], Dkl_per_step[:step_idx+1])

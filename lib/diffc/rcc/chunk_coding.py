...
def chunk_and_encode(mu, chunk_sizes, shared_seed=0):
    ...
    for i, chunk_size in enumerate(chunk_sizes):
        ...
        seed, partition = reverse_channel_encode(mu_chunk, K=int(2**chunk_size), shared_seed=chunk_shared_seed)
        seeds.append(seed)
...
def decode_from_chunks(dim, seeds, chunk_sizes, shared_seed=0):
    ...
    for i, seed in enumerate(seeds):
        ...
        partition = reverse_channel_decode(chunk_dim, seed, shared_seed=chunk_shared_seed)
        partitions.append(partition)
